...
class Graph:
    ...
    def df_print(self):
        node_dfs = []
        for node in self._nodes.values():
            node_dfs.append(node.df_print())
        node_dfs = pd.concat(node_dfs).set_index('node')

        edge_dfs = []
        for edge in self._edges:
            edge_dfs.append(edge.df_print())
        edge_dfs = pd.concat(edge_dfs).set_index('edge')
...
class Node:
    ...
    def df_print(self):
        return pd.DataFrame({'node': [self._name],'dist': [self.dist], 'excess': [self.excess]})
...
class Edge:
    ...
    def df_print(self):
        ...
        return pd.DataFrame({'edge': [(self._node1._name, self._node2._name)], 'load': [self.load], 'capacity': [self.capacity]})
